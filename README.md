...
- **Prompt attribution.** Show which person submitted each prompt in the conversation thread so the full history reads like a real conversation with clear accountability.
...
- **Persistent task logs.** Task logs currently rely on a WebSocket connection that can die. Logs should be cached server-side and served from cache on reconnect so you never lose visibility.
- **Multiplayer plan mode.** Extend draft/plan mode into a full collaborative workflow. The agent publishes a plan with rendered Markdown. Team members comment, request changes, approve or reject. Comments are fed back to the agent, which can regenerate the plan. Once approved, the agent executes.
